...
function downloadMallInvoice(mallId: DownloadMallId, mallLabel: string, orders: ShippedOrder[]) {
  ...
  if (mallId === 'tossshopping') {
    ...
    mallOrders.forEach(o => {
      const entry = { tracking: digitsOnly(o.tracking_number) }
    ...
    /* ── 2. 가장 최근 업로드된 raw AOA 선택 ── */
    const sorted = [...allDayData].sort((a, b) =>
      (b.uploaded_at ?? '').localeCompare(a.uploaded_at ?? '')
    )
    const tmpl = sorted.find(d => d.toss_raw_aoa && d.toss_raw_aoa.length >= 2)

    if (tmpl?.toss_raw_aoa) {
      const aoa = tmpl.toss_raw_aoa
      ...
      if (hdrIdx >= 0) {
        const headerRow = aoa[hdrIdx] as unknown[]
        ...
        const wb = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(wb, ws, sheetName)
        const outBuf = XLSX.write(wb, { bookType: 'xlsx', type: 'array' }) as ArrayBuffer
        const blob = new Blob([outBuf], { type: 'application/octet-stream' })
        const url  = URL.createObjectURL(blob)
        const a    = document.createElement('a')
        a.href     = url
        a.download = `${mallLabel}_송장_${todayStr()}.xlsx`
        a.click()
        URL.revokeObjectURL(url)
        return
      }
    ...
    /* ── 4. 최종 폴백: 배송처리된 주문 기본 정보 ── */
    triggerExcelDownload(
      mallOrders.map(o => ({
        '주문번호': o.order_number, '주문상태': '배송중',
        '택배사': 'CJ대한통운', '송장번호': digitsOnly(o.tracking_number),
        '수취인': o.customer_name,
      })),
      `${mallLabel}_송장_${todayStr()}.xlsx`
    )
    return
  }
    ...
    <div style={{ maxWidth: 1080, margin: '0 auto' }}>
      ...
      <div className="pm-card" style={{ padding: '16px 20px', marginBottom: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
          ...
          <span style={{ fontSize: '13.5px', fontWeight: 800, color: '#0f172a' }}>쇼핑몰별 송장 파일 다운로드</span>
          <span style={{ fontSize: '11.5px', color: '#94a3b8' }}>· 배송처리된 주문 기준</span>
        </div>
            ...
            return (
              ...
              >
                ...
                <div style={{ textAlign: 'left' }}>
                  <p style={{ fontSize: '13px', fontWeight: 800, color: mall.color, margin: 0 }}>{mall.label}</p>
                  <p style={{ fontSize: '11px', color: '#94a3b8', marginTop: 2 }}>{count > 0 ? `${count}건 대기` : '주문 없음'}</p>
                </div>
              </button>
            )
          })}
      ...
      <div className="pm-card" style={{ padding: '10px 16px', marginBottom: 14, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        <input value={search} onChange={e => setSearch(e.target.value)}
          ...
          style={{ flex: 1, height: 34, fontSize: '13px', border: 'none', outline: 'none', background: 'transparent', minWidth: 140 }}
        />
        ...
        {!showAllDates && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
            ...
            <div style={{ padding: '4px 12px', borderRadius: 8, background: dateFilter === today ? '#eff6ff' : '#f8fafc', border: `1px solid ${dateFilter === today ? '#bfdbfe' : '#e2e8f0'}`, minWidth: 170, textAlign: 'center' }}>
              <span style={{ fontSize: '13px', fontWeight: 800, color: '#0f172a' }}>{fmtDateKo(dateFilter)}</span>
              {dateFilter === today && <span style={{ fontSize: '10px', fontWeight: 900, color: '#2563eb', background: '#dbeafe', padding: '1px 6px', borderRadius: 20, marginLeft: 6 }}>TODAY</span>}
            </div>
            <button onClick={() => setDateFilter(d => shiftDate(d, 1))}
        ...
        {checked.size > 0 && (
          <>
            <span style={{ fontSize: '12px', fontWeight: 800, color: '#2563eb', background: '#eff6ff', padding: '5px 10px', borderRadius: 8 }}>
              {checked.size}건 선택
            </span>
            <button
      ...
      <div className="pm-card" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '12px 20px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: '13.5px', fontWeight: 800, color: '#0f172a' }}>송장 등록 완료 주문</span>
            ...
            {filtered.map(order => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ fontSize: '12px', fontFamily: 'monospace', color: '#334155', fontWeight: 700 }}>{order.tracking_number}</span>
                  ...
                  <button
                    ...
                    style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, padding: '5px 8px', background: '#7c3aed', color: '#fff', border: 'none', borderRadius: 7, fontSize: '11px', fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap' }}

...
function MonthNav({ month, setMonth }: { month: string; setMonth: (m: string) => void }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:5 }}>
      ...
      <span style={{ fontSize: '12px', fontWeight:800, color:'#0f172a', minWidth:80, textAlign:'center', whiteSpace:'nowrap' }}>
        {fmtMonthLabel(month)}
      </span>
...
export default function PurchaseMainPage() {
  ...
  const thStyle = (align: 'left'|'center' = 'center'): React.CSSProperties => ({
    padding:'6px 8px', fontWeight:800, color:'#64748b', fontSize: '10.5px', textAlign:align,
    ...
    <div className="pm-page" style={{ display:'flex', flexDirection:'column', height:'100%', gap:0 }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, flex:1, overflow:'hidden' }}>
          ...
          <div className="pm-card" style={{ padding:'10px 14px', flexShrink:0 }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6, marginTop:10 }}>
              ...
              <div style={{ background:'#f8fafc', borderRadius:8, padding:'6px 10px' }}>
                <p style={{ fontSize: '9.5px', fontWeight:800, color:'#94a3b8' }}>발주 수량</p>
                <p style={{ fontSize: '18px', fontWeight:900, color:'#1e293b', lineHeight:1 }}>{poOrderedQty.toLocaleString()}</p>
              </div>
              <div style={{ background:'#fefce8', borderRadius:8, padding:'6px 10px' }}>
                <p style={{ fontSize: '9.5px', fontWeight:800, color:'#94a3b8' }}>발주금액 (월누적)</p>
          ...
          <div className="pm-card" style={{ padding:'8px 12px', flexShrink:0, display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ position:'relative', flex:1 }}>
              ...
              <input
                ...
                style={{ width:'100%', height:32, paddingLeft:30, paddingRight:searchText ? 30 : 10, fontSize: '12px', fontWeight:600, border:'1.5px solid #e2e8f0', borderRadius:8, outline:'none', color:'#0f172a', boxSizing:'border-box' }}
              />
              {searchText && (
            ...
            {searchText && (
              <span style={{ fontSize: '11px', fontWeight:700, color:'#2563eb', background:'#eff6ff', padding:'3px 8px', borderRadius:6, whiteSpace:'nowrap' }}>
          ...
          <div className="pm-card" style={{ flex:1, overflow:'auto', padding:0 }}>
            {poList.length === 0
              ? <div style={{ textAlign:'center', padding:'40px 0', color:'#94a3b8' }}>
                  ...
                  <p style={{ fontSize: '13px', fontWeight:700 }}>이 달의 발주 내역이 없습니다</p>
                </div>
                        ...
                        <>
                          ...
                          >
                            <td style={tdStyle('left')}>
                              ...
                              <span style={{ display:'block', fontSize: '10px', color:'#94a3b8' }}>{p.order_date}</span>
                            </td>
                            <td style={{ ...tdStyle('left'), fontSize: '11.5px', color:'#475569' }}>{p.supplier||'-'}</td>
                            <td style={{ ...tdStyle(), color:'#64748b' }}>{p.items.length}건</td>
                            <td style={{ ...tdStyle(), fontWeight:800, color:'#1e293b' }}>{tOrd.toLocaleString()}</td>
                            <td style={{ ...tdStyle(), fontWeight:900, color: lineKrw > 0 ? '#92400e' : '#cbd5e1', fontSize: '11.5px' }}>
                                    ...
                                    {p.items.map((item, i) => {
                                      ...
                                      return (
                                        <tr key={i} style={{ borderBottom:'1px solid #e0f2fe', background: isEditing ? '#fafffe' : isItemDel ? '#fef2f2' : undefined, opacity: isItemDel ? 0.65 : 1 }}>
                                          ...
                                          <td style={{ padding:'4px 8px', color:'#334155', fontFamily:'monospace', fontSize: '10.5px' }}>{item.product_code}</td>
                                          <td style={{ padding:'4px 8px', color:'#475569', fontSize: '10.5px' }}>{item.option_name||'-'}</td>
                                          <td data-pm-barcode="1" style={{ padding:'4px 8px', fontFamily:'monospace', fontSize: '10px' }}>{item.barcode||'-'}</td>
                                          {/* 발주수량 - 편집 모드 */}
                                          <td style={{ padding:'4px 8px', textAlign:'center' }}>
                                            {isEditing ? (
                                              <input
                                                ...
                                                style={{ width:58, height:24, fontSize: '11.5px', fontWeight:800, textAlign:'center', border:'1.5px solid #3b82f6', borderRadius:6, outline:'none', padding:'0 4px' }}
                                              />
                                            ) : (
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10, overflow:'hidden' }}>
          ...
          <div className="pm-card" style={{ padding:'10px 14px', flexShrink:0 }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
              <span style={{ fontSize: '14px', fontWeight:900, color:'#0f172a' }}>🚚 미입고 리스트</span>
              <span style={{ fontSize: '11px', color:'#94a3b8' }}>전체 {miItems.length}건</span>
            </div>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:6 }}>
              <div style={{ background:'#fff7ed', borderRadius:8, padding:'6px 10px' }}>
                <p style={{ fontSize: '9.5px', fontWeight:800, color:'#94a3b8' }}>미입고 품목 수</p>
                <p style={{ fontSize: '18px', fontWeight:900, color:'#d97706', lineHeight:1 }}>{miItems.length}</p>
              </div>
              <div style={{ background:'#f8fafc', borderRadius:8, padding:'6px 10px' }}>
                <p style={{ fontSize: '9.5px', fontWeight:800, color:'#94a3b8' }}>미입고 수량</p>
                <p style={{ fontSize: '18px', fontWeight:900, color:'#1e293b', lineHeight:1 }}>{miTotalQty.toLocaleString()}</p>
              </div>
            </div>
          </div>
          ...
          <div className="pm-card" style={{ flex:1, overflow:'auto', padding:0 }}>
            {miItems.length === 0
              ? <div style={{ textAlign:'center', padding:'40px 0', color:'#94a3b8' }}>
                  ...
                  <p style={{ fontSize: '13px', fontWeight:700 }}>미입고 항목이 없습니다</p>
                </div>
              : <table style={{ width:'100%', borderCollapse:'collapse', fontSize: '12px' }}>
                  <thead style={{ position:'sticky', top:0, zIndex:1 }}>
                    <tr style={{ background:'#f8fafc' }}>
                  ...
                  <tbody>
                    {miItems.map((row, idx) => {
                      ...
                      >
                        ...
                        <td data-pm-barcode="1" style={{ ...tdStyle(), fontFamily:'monospace', fontSize: '10.5px', whiteSpace:'nowrap' }}>
                          {row.barcode || '-'}
                        </td>
                        <td style={{ ...tdStyle('left'), fontFamily:'monospace', fontSize: '11px', color:'#059669', fontWeight:800 }}>
                          {row.productCode}
                        </td>
                        <td style={{ ...tdStyle('left'), fontSize: '11px', color:'#475569', maxWidth:100 }}>
                        ...
                        <td style={{ ...tdStyle() }}>
                          <span style={{ fontSize: '12px', fontWeight:900, background:'#fff7ed', color:'#d97706',
